...
bool ToolTranslate::Start(ICamera *pCam, int x, int y, bool bLockX, bool bLockY, bool bLockZ)
{
	if(m_bForced)
	{
		...
		if(!g_Map->GetHeightmap()->RayIntersect(vOrigin,vDir,vIntersect))
		{
			m_bValidPlace=false;
			m_pObj->GetTransformation().SetPosition(vOrigin + vDir*30.0f);
		}
		else
		{
			if(m_pObj->GetObjectType()==OBJ_MAP)
				m_bValidPlace=!g_Map->GetObjectMap()->IsPlaceUsed((int)vIntersect.x,(int)vIntersect.z,((IMapObject *)m_pObj)->GetRadius());
			m_pObj->GetTransformation().SetPosition(vIntersect);
		}

		m_pObj->GetTransformation().RebuildMatrix();

		if(m_bValidPlace && m_pObj->GetObjectType()==OBJ_MAP)
		{
			((IMapObject *)m_pObj)->SetMapPos(Core::Vector2i((int)vIntersect.x,(int)vIntersect.z));
		}
		else if(m_pObj->GetObjectType()!=OBJ_MAP)
			g_Map->AddObject(m_pObj);
	}
	else
	...
	if(m_pObj->GetObjectType()==OBJ_MAP)
	{
		m_vSavedPos=m_pObj->GetTransformation().GetPosition();
		m_vSavedMapPos=((IMapObject *)m_pObj)->GetMapPos();
	}
	...
	else
	{
		...
		if(m_pCam->GetType()==CAM_PERSPECTIVE)
		{
			...
			if(plane.IntersectLine(vOrigin,vDir,newpos))
			{
				...
				if(m_pObj->GetObjectType()==OBJ_EDITOR)
					((IEditorObject *)m_pObj)->Translate(translate);
				else
				{
					m_pObj->GetTransformation().Translate(translate);
					m_pObj->GetTransformation().RebuildMatrix();
					g_Map->UpdateObject(m_pObj);
				}
		...
		else
		{
			...
			if(m_pObj->GetObjectType()==OBJ_EDITOR)
				((IEditorObject *)m_pObj)->Translate(v);
			else
			{
...
{
	if(!m_pObj || m_pObj->GetObjectType()!=OBJ_MAP) return;

	if(m_bValidPlace)
		m_pObj->DrawAmbient();
	else
	{
		if(m_hInvalidShader.IsNull())
			m_hInvalidShader=g_ShaderMgr->LoadShader("StdInvalidPlace");

		Core::HMesh hMesh=((IMapObject *)m_pObj)->GetMesh();
		IMesh *pMesh=g_MeshMgr->GetMesh(hMesh);

		if(!pMesh) return;

		g_ShaderMgr->SetShader(m_hInvalidShader);
		...
		g_Renderer->SetRenderState(D3DRS_ZFUNC,D3DCMP_ALWAYS);
		g_Renderer->SetRenderState(D3DRS_CULLMODE,D3DCULL_NONE);
		g_Renderer->SetTransformation(TRANS_MODEL,m_pObj->GetTransformation().GetMatrix(),0,false);
		g_Renderer->SetVertexBuffer(pMesh->GetVertexBuffer(),pMesh->GetVerticesCount(),sizeof(Core::VertexPNDTT));
		g_Renderer->SetIndexBuffer(pMesh->GetIndexBuffer());
		g_Renderer->RenderStaticBuffers(D3DPT_TRIANGLELIST,0,pMesh->GetTriCount());
...
int ToolTranslate::TranslateMapObject()
{
	Core::Vector3 vOrigin,vDir;
	m_pCam->MakeRayFrom2D(m_iLastStartX+m_iTotalMoveX,m_iLastStartY+m_iTotalMoveY,vOrigin,vDir);
	...
	if(m_bValidPlace)
		...
		g_Map->GetObjectMap()->RemoveObject((IMapObject*)m_pObj);
...
{
	if(m_bForced && m_pObj->GetObjectType()!=OBJ_EDITOR) // l'objet n'a pas été placé
	{
		g_Map->GetObjectFactory()->ReleaseObject(m_pObj);
		m_bForced=false;
	}
	else if(m_pObj && m_pObj->GetObjectType()==OBJ_MAP)
	{
		if(!m_bValidPlace)
		{
			m_pObj->GetTransformation().SetPosition(m_vSavedPos);
			((IMapObject *)m_pObj)->SetMapPos(m_vSavedMapPos);
		}

		g_Map->GetObjectMap()->AddObject((IMapObject *)m_pObj);

		m_bValidPlace=true;
	}
}

void ToolTranslate::ForceObject(Game::IObject *pObj)
{
	if(pObj)
	{
		m_pObj=pObj;
